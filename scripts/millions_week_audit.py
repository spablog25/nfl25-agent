...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--fail_on_missing", type=float, default=0.01,
                   help="Max allowed missing ratio in key fields (e.g., 0.01 for 1%)")
    p.add_argument("--spread_range", type=str, default="-20,20",
                   help="Min,max allowed spreads")
    p.add_argument("--dvoa_range", type=str, default="-0.6,0.6",
...
def audit(planner: pd.DataFrame,
          season: int,
          week: int,
          ...
          spread_max: float,
          dvoa_min: float,
          dvoa_max: float) -> tuple[int, str, pd.DataFrame]:
    ...
    if "circa_line" in planner.columns:
        bad_spread = planner[~planner["circa_line"].between(spread_min, spread_max, inclusive="both") & planner["circa_line"].notna()]
        ...
        if not bad_spread.empty:
            ...
            for idx, r in bad_spread.iterrows():
                flags_rows.append({"row": int(idx), "field": "circa_line", "issue": "out_of_range", "value": r["circa_line"]})
        lines.append("")

    # 4) Ranges: DVOA projections
    for col in ["team_total_dvoa_proj", "opp_total_dvoa_proj"]:
        if col in planner.columns:
            bad = planner[~planner[col].between(dvoa_min, dvoa_max, inclusive="both") & planner[col].notna()]
            lines.append(f"## {col} range check [{dvoa_min},{dvoa_max}] — bad: {len(bad)}")
            if not bad.empty:
                severity = max(severity, 2)
                for idx, r in bad.iterrows():
                    flags_rows.append({"row": int(idx), "field": col, "issue": "out_of_range", "value": r[col]})
            lines.append("")

    # 5) Alias resolution
    if alias_set is not None:
        missing_alias_team = planner[~planner["team"].isin(alias_set)] if "team" in planner.columns else pd.DataFrame()
        missing_alias_opp = planner[~planner["opponent"].isin(alias_set)] if "opponent" in planner.columns else pd.DataFrame()
        ...
        if total_miss > 0:
            ...
            for idx, r in missing_alias_team.iterrows():
                flags_rows.append({"row": int(idx), "field": "team", "issue": "alias_unresolved", "value": r.get("team")})
            for idx, r in missing_alias_opp.iterrows():
                flags_rows.append({"row": int(idx), "field": "opponent", "issue": "alias_unresolved", "value": r.get("opponent")})
        lines.append("")
    ...
    if "home_or_away" in planner.columns:
        ...
        if not invalid.empty:
            ...
            for idx, r in invalid.iterrows():
                flags_rows.append({"row": int(idx), "field": "home_or_away", "issue": "invalid_value", "value": r.get("home_or_away")})
        lines.append("")
    ...
    if dup_cols:
        ...
        if not dup_rows.empty:
            ...
            for idx, r in dup_rows.iterrows():
                flags_rows.append({"row": int(idx), "field": ",".join(dup_cols), "issue": "duplicate_row"})
        lines.append("")

    # 8) Survivor cross-check (optional light sanity)
    if survivor is not None and not survivor.empty:
        # Only a light key presence check; deeper semantic checks can come later
        for c in ["team", "opponent"]:
            if c in survivor.columns:
                survivor[c] = survivor[c].astype(str).str.strip().str.upper()
        # No failure here by default, just counts
